...
class Utils {
  ...
  getInstalledStatus(pkgName, targetDir) {
    ...
    if (JSON.stringify(current) === lts.trim()) return 2
    return 1
  }
  ...
  getInstalledGenerators(targetDir) {
    ...
    Object.keys(dependencies).forEach(v => {
      if (!v.match(/^mf-/)) delete dependencies[v]
    })
    return dependencies
  }
  ...
  getConfigs() {
    const configs = this.requireFrom(process.cwd(), "./mconfig.js")
    if (!configs || !configs.builder) {
      this.console(
        "请确保工程根路径下有 mconfig.js 文件，且文件中配置了 builder 属性",
        "red"
      )
      process.exit(1)
    }
